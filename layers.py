...
class Embedding(nn.Module):
    ...
    def forward(self, x_w, x_c):
        ...
        # # for sentence in x_w:
        # for i in range(0, x_w.size()[0]):
        #     sentence_pos = []
        #     for j in range(0, x_w.size()[1]):
        #         word = x_w[i][j]
        #     # for word in sentence:
        #         # print("pos: ", self.wordidx2posidx[str(int(word))])
        #         pos = torch.tensor([self.wordidx2posidx[str(int(word))]])
        #         # print(pos.size(), word.size())
        #         sentence_pos.append(pos)
        #         # torch.cat((word, pos))
        #     pos_list.append(sentence_pos)

        # pos_list = [[self.wordidx2posidx[str(int(word))] for word in sentence] for sentence in x_w]

        # print(len(pos_list), len(pos_list[0]))

        # x = torch.tensor(pos_list)
        ...
        print(x.size())
        print("emb_w size: ", emb_w.size())
        print("emb_c size: ", x_conv_out_c.size())
        print("x size: ", x.size())
        concatenated = torch.cat((x_conv_out_c, emb_w, x), 1)
        print("concatendated: ", concatenated.size())
